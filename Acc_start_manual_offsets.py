...
for i, off in enumerate(best_offsets):
    plt.scatter([off], [i], s=200)
    plt.text(off + 0.2, i, f"+{off} days", va="center")
plt.yticks(range(len(best_offsets)), [f"Account {i+1}" for i in range(len(best_offsets))])
plt.title("Start Offsets")
