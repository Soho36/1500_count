...
TRAIL_STOP_THRESHOLD = MAX_DD + 100  # define the trailing stop cutoff
...
while start_idx_pointer < len(start_indices):
    ...
    for i in range(start_idx, len(dataframe)):
        ...
        # --- Update DD logic ---
        if USE_TRAILING_DD:
            # check if trailing still active
            if trailing_active:
                # update peak depending on available columns
                ...
                # compute trailing floor normally
                trailing_floor = peak_pnl - MAX_DD

                # check if we must stop trailing
                if cumulative_pnl > TRAIL_STOP_THRESHOLD:
                    trailing_active = False
                    frozen_trailing_floor = trailing_floor

                # use active trailing floor
                dd_limit = trailing_floor

            else:
                # trailing stopped — use frozen limit
                dd_limit = frozen_trailing_floor
